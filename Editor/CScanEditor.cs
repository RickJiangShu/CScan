...
public class CScanEditor : Editor{
    ...
    static void Scan(bool ignore)
    {
        ...
        UnityEngine.Debug.Log(string.Format("共扫描{0}个文件，{1}总共：{2}行\n选取文件耗时：{3}s，扫描耗时：{4}s", count, scanName, lineCount, selectSeconds.ToString("0.000"), scanSeconds.ToString("0.000")));
    }
